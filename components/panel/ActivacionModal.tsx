...
export function ActivacionModal({
  activation,
  onClose,
}: {
  ...
  const [url, setUrl] = useState(activation?.youtubeUrl ?? "");
  const [title, setTitle] = useState(activation?.title ?? "");
  const [occurredAt, setOccurredAt] = useState(activation?.occurredAt ?? "");
  const [error, setError] = useState("");
  const [pending, setPending] = useState(false);
  const [lookingUp, setLookingUp] = useState(false);
  const lastLookup = useRef(activation?.youtubeUrl ?? "");
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center px-4 py-6">
      ...
      <form
        ...
        action={async (formData) => {
          setPending(true);
          setError("");
          const action = activation ? updateActivationAction : createActivationAction;
      ...
      >
        {activation ? <input type="hidden" name="id" value={activation.id} /> : null}
        <input type="hidden" name="enabled" value={activation ? String(activation.enabled) : "true"} />
        <div className="px-6 pt-6">
          <div className="flex items-start justify-between gap-4">
            <h2 className="font-heading text-3xl text-primary uppercase">
              {activation ? "Editar activación" : "Agregar activación"}
            </h2>
        ...
        <div className="flex justify-end gap-2 px-6 pb-6">
          ...
          >
            {pending ? "Guardando..." : "Guardar activación"}
          </button>
        </div>
      </form>
    </div>
